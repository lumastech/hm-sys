        ...
        <template #content>
            <div class="p-4 md:p-5 space-y-4">
                <form @submit.prevent="submitForm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            <label for="attended_by" class="block text-sm font-medium text-gray-700">Assign To</label>
                    ...
                    <div class="mb-4">
                        <label for="image" class="block text-sm font-medium text-gray-700">Attachment</label>
                        <input type="file" id="image" @change="form.image = $event.target.files[0]"
                            class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                        <div v-if="helpdesk.image" class="mt-2">
                            <p class="text-sm text-gray-500">Current attachment:</p>
                            <img :src="`/storage/${helpdesk.image}`" class="h-20 mt-1 rounded">
                        </div>
                    </div>
                </form>
            </div>
...
const form = useForm({
    ...
    phone: props.helpdesk.phone,
    email: props.helpdesk.email,
    subject: props.helpdesk.subject,
    description: props.helpdesk.description,
    attended_by: props.helpdesk.attended_by,
    status: props.helpdesk.status,
    image: null,
});

const submitForm = () => {
    form.put(route('helpdesks.update', props.helpdesk.id), {
        onSuccess: () => {
            emit('close');
        },
    });
